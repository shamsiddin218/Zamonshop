      ...
      <NavLink to={'/watch'}>
          ...
          <article  className='w-full grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-[20px] sm:gap-[24px] lg:gap-[30px]'>
            ...
            <article key={product.id} className='w-full max-w-[270px] m-auto border border-[#80808055] rounded-xl overflow-hidden cursor-pointer transition-all duration-200 relative hover:shadow-md group'>
              <article onClick={() => toggleLike(product)} className=' p-[7px] bg-[#acababd6] rounded-[8px] absolute right-1 top-1 z-20'>
              <IoMdHeart color={isLiked(product.id) ? "blue" : "white"}  className=' text-[white] text-[20px]'/>
                ...
                <article className=' w-full h-[280px] xs:h-[300px] sm:h-[309px] overflow-hidden'>
                    <img className='w-full h-full object-cover group-hover:scale-105 transition-all duration-200' src={product.image} alt="" />
                </article>
                <article className=' w-full h-full p-[5px]'>
                    ...
                    <h6 className='line-clamp-2 mb-[8px] text-[14px] sm:text-[15px] md:text-[16px] '>{product.title} </h6>
                    <button onClick={() => setSelectedProduct(product)} className=' w-full py-[6px] sm:py-[8px] bg-blue-600 hover:bg-blue-700 transition-colors duration-150 text-white text-[14px] sm:text-[15px] md:text-[16px] flex justify-center items-center rounded-[10px]'>Ko'rish <MdAddShoppingCart/></button>
                </article>
            </article>
            ))}
          </article>
