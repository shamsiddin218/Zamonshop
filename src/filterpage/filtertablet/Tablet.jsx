...
export default function Tablet({handleAddToCart}) {
    const tablet = data.Alldata.filter(item => item.key === "Planshet").sort(()=> 0.5 - Math.random())
    const [visibleCount, setVisibleCount] = useState(12); 
          ...
          const filteredProducts = selectedCategory === "Barchasi"
            ? tablet
            : tablet.filter(item => item.category === selectedCategory);
  ...
  <div className=" max-w-[1200px] m-auto mb-[44px] px-[10px] mt-[125px]">
          <article className=' w-full flex justify-between items-center'>
            ...
            <select value={selectedCategory} onChange={handleCategoryChange} className=' border border-gray-400 bg-transparent  rounded-md outline-none py-[3px] px-[6px]'>
                  ...
                  <option className=' dark:bg-black' value="Planshet giloflari">Planshet g'iloflari</option>
                  <option className=' dark:bg-black' value="iPadlar">iPadlar</option>
                  <option className=' dark:bg-black' value="Sovutgich">Sovutgich</option>
                  <option className=' dark:bg-black' value="Elektron kitoblar">Elektron kitoblar</option>
          ...
          <article className="w-full grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-[20px] sm:gap-[24px] lg:gap-[30px] gap-y-[30px] mb-[30px]">
            {tablet.map((tab)=>(
            <article key={tab.id} className="w-full max-w-[270px] m-auto border border-[#80808055] rounded-xl overflow-hidden cursor-pointer transition-all duration-200 relative hover:shadow-md group">
              ...
              <article className=" w-full h-full p-[5px]">
                ...
                <button onClick={() => setSelectedProduct(tab)} className=" w-full py-[6px] sm:py-[8px] bg-blue-600 hover:bg-blue-700 transition-colors duration-150 text-white text-[14px] sm:text-[15px] md:text-[16px] flex justify-center items-center rounded-[10px]">
