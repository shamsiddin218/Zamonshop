...
export default function Phonefilter({handleAddToCart}) {
    ...
    const [visibleCount, setVisibleCount] = useState(12); 
      ...
      const filteredProducts = selectedCategory === "Barchasi"
        ? phone
        : phone.filter(item => item.category === selectedCategory);
    ...
    <div className=" max-w-[1200px] m-auto mb-[44px] px-[10px] mt-[125px]">
          <article className=' w-full flex justify-between items-center'>
            ...
            <select value={selectedCategory} onChange={handleCategoryChange} className=' border border-gray-400 bg-transparent  rounded-md outline-none py-[3px] px-[6px]'>
                  ...
                  <option className=' dark:bg-black' value="Samsung">Samsung</option>
                  <option className=' dark:bg-black' value="iPhone">Iphone</option>
                  <option className=' dark:bg-black' value="Honor">Honor</option>
                  <option className=' dark:bg-black' value="Redmi">Redmi</option>
                  <option className=' dark:bg-black' value="Poco">Poco</option>
                  <option className=' dark:bg-black' value="Infinix">Infinix</option>
                  <option className=' dark:bg-black' value="Tecno">Tecno</option>
                  <option className=' dark:bg-black' value="Vivo">Vivo</option>
                  <option className=' dark:bg-black' value="Knopkali telefonlar">Knopkali telefonlar</option>
                </select>
          </article>
